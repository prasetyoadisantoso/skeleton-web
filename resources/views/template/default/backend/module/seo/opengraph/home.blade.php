    ...
    <div class="card" id="meta-home">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                ...
                <a href="{{route('opengraph.create')}}" class="btn btn-success">{{$button['create']}}<i
...
@push('opengraph-home-js')
<script type="text/javascript" alt="datatable">
    $("#opengraph_datatable").DataTable({
        scrollX: true,
        processing: true,
        ajax: "{{route('opengraph.datatable')}}",
        dom: 'lfrtip',
        paging: true,
        pageLength: 5,
        lengthMenu: [ 5, 10, 25 ],
        columns: [
            ...
            { data: 'url', name: 'url', searchable: true, orderable: true },
            {
                data: 'action', name: 'action', orderable: false, searchable: false, render: function (data, type, full, meta) {
                    var id = data;
                    // Edit
                    var edit = "{{route('opengraph.edit', ':id')}}";
                    edit = edit.replace(':id', id);
                    // Delete
                    var destroy = "{{route('opengraph.destroy', ':id')}}";
