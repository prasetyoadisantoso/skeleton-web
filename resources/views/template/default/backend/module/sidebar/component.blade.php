...
<li class="py-2 shadow-sm">
    {{-- Link dropdown --}}
    <a href="#template-dropdown-menu" data-bs-toggle="collapse" aria-expanded="" {{-- Set aria-expanded berdasarkan state aktif --}}
        ...
        <div class="d-flex align-items-center main-list" id="rotation-template"> {{-- ID unik --}}
            ...
            <span class="flex-grow-1 font-md"><i class="fa-solid fa-layer-group me-3"></i>{{$sidebar['template'] ?? 'Template'}}</span> {{-- Ganti key translasi --}}
            {{-- Ikon chevron --}}
    ...
    <ul class="collapse list-unstyled mx-5 mt-3 " id="template-dropdown-menu"> {{-- ID unik & class 'show' --}}
        ...
        <li class="sub-list font-sm"> {{-- Tambahkan class active-sub jika perlu styling khusus --}}
            <a href="{{route('component.index')}}" class="text-white">
                <span><i class="fa-solid fa-circle-dot me-3"></i>{{$sidebar['component'] ?? 'Component'}}</span> {{-- Ganti key translasi jika perlu --}}
            </a>
        </li>
        @endcan
        ...
        <li class="sub-list font-sm"> {{-- Tambahkan class active-sub jika perlu styling khusus --}}
            <a href="{{route('page.index')}}" class="text-white"> {{-- Ganti dengan route page.index (buat jika belum ada) --}}
                <span><i class="fa-solid fa-circle-dot me-3"></i>{{$sidebar['page'] ?? 'Page'}}</span> {{-- Ganti key translasi (buat jika belum ada) --}}
            </a>
        </li>
        @endcan
