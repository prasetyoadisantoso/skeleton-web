...
<div class="container py-2">
    ...
    <!-- Start app -->
    <div class="card" id="tag-home">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                <h5 class="align-self-center">{{$datatable['header']['title']}}</h5>
                <a href="{{route('tag.create')}}" class="btn btn-success">{{$button['create']}}<i
                        class="fa-solid fa-plus ms-3"></i></a>
            </div>
        </div>
        <div class="card-body">
            <table id="tag_datatable" class="table table-bordered w-100">
                <thead>
                    <tr>
        ...
        columns: [
            ...
            { data: 'slug', name: 'slug', searchable: true, orderable: true },
            {
                data: 'action',  width: '25%', name: 'action', orderable: false, searchable: false, render: function (data, type, full, meta) {
                    var id = data;
                    // Edit
                    var edit = "{{route('tag.edit', ':id')}}";
                    edit = edit.replace(':id', id);
                    // Delete
                    var destroy = "{{route('tag.destroy', ':id')}}";
