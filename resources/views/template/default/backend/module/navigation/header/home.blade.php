...
@section('headermenu-home')

<!-- Start Breadcrumb -->
<nav class="bg-light py-3 px-2 px-md-5 shadow-sm d-flex justify-content-between" aria-label="breadcrumb">
    <h5 class="my-0"><i class="fa-solid fa-list me-3"></i>{{$breadcrumb['title'] ?? 'Header Menu' }}</h5>
...
<div class="container py-2">
    ...
    <!-- Start app -->
    <div class="card" id="headermenu-home">
        <div class="card-header">
            <div class="d-flex justify-content-between">
                <h5 class="align-self-center">{{$datatable['header']['title'] ?? 'List Menu'}}</h5>
                <a href="{{route('headermenu.create')}}" class="btn btn-success">{{$button['create'] ?? 'Create'}}<i
                        class="fa-solid fa-plus ms-3"></i></a>
            </div>
        </div>
        <div class="card-body">
            <table id="headermenu_datatable" class="table table-bordered w-100">
                <thead>
                    <tr>
                        ...
                        <th>{{$datatable['table']['parent'] ?? "Parent"}}</th>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{$datatable['table']['parent'] ?? "Parent"}}</td>
...
@push('headermenu-home-js')
<script type="text/javascript" alt="datatable">
    $("#headermenu_datatable").DataTable({
        scrollX: true,
        processing: true,
        ajax: "{{route('headermenu.datatable')}}",
        dom: 'lfrtip',
        paging: true,
        pageLength: 5,
        lengthMenu: [ 5, 10, 25 ],
        columns: [
            ...
            { data: 'status', name: 'status', searchable: true, orderable: true },
            {
                data: 'action',  width: '25%', name: 'action', orderable: false, searchable: false, render: function (data, type, full, meta) {
                    var id = data;
                    // Show
                    var show = "{{route('headermenu.show', ':id')}}";
                    show = show.replace(':id', id);
                    // Edit
                    var edit = "{{route('headermenu.edit', ':id')}}";
                    edit = edit.replace(':id', id);
                    // Delete
                    var destroy = "{{route('headermenu.destroy', ':id')}}";
                    ...
                    success: function (results) {
                        if (results.status === 'success') {
                            Swal.fire({
                                ...
                                $('#headermenu_datatable').DataTable().ajax.reload();
                            });
                        } else {
                            Swal.fire({
