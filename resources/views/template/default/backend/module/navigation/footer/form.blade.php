...
@section('footermenu-form')

<!-- Start Breadcrumb -->
<nav class="bg-light py-3 px-2 px-md-5 shadow-sm d-flex justify-content-between" aria-label="breadcrumb">
    <h5 class="my-0"><i class="fa-solid fa-signs-post me-3"></i>{{$breadcrumb['title'] ?? "Footer Menu Management"}}
    </h5>
    ...
    <div class="card" id="category-create">
        ...
        <div class="card-body">
            ...
            <div class="container-fluid">
                ...
                <form action="{{ $type == 'edit' ? route('footermenu.update', $menu) : route('footermenu.store') }}"
                    method="POST" id={{ $type=='create' ? 'footermenu-store-form' : 'footermenu-update-form' }}>
                    @csrf
                    @if($type == 'edit')
                    @method('PUT')
                    @endif
        ...
        <div class="card-footer">
            <div class="d-flex justify-content-center">
                @if ($type == 'create')
                @can ("footermenu-store")
                <button id="footermenu-store-submit" type="submit" class="btn btn-success w-100 w-md-25">
                ...
                @if ($type == 'edit')
                @can ("footermenu-update")
                <button id="footermenu-update-submit" type="submit" class="btn btn-success w-100 w-md-25">
...
<script alt="jquery-validation">
    ...
    $('#footermenu-store-submit').click(function () {
        let a = $("#footermenu-store-form").valid();
        if (a === true) {
            $("#footermenu-store-form").submit();
        } else {
            return false;
        }
    });

    $('#footermenu-update-submit').click(function () {
        let a = $("#footermenu-update-form").valid();
        if (a === true) {
            $("#footermenu-update-form").submit();
        } else {
            return false;
        }
    });
